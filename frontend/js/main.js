...
function loadDoctorsBySpec() {
  ...
  document.getElementById('book-fees').value = '';
  document.getElementById('book-docemail').value = '';

  if (!spec) return;

  const filtered = allDoctorsGlobal.filter(d => d.spec === spec);
  filtered.forEach(d => {
    docSelect.innerHTML += `<option value="${d.username}">${d.username}</option>`;
  });
}
...
function updateFeesAndEmail() {
  ...
  if (doctor) {
    feesInput.value = doctor.docFees;
...
async function handleAddDoctor(event) {
  ...
  try {
    const res = await fetch(`${BASE_URL}/doctors/add`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ username, email, spec, docFees, password })
    });
